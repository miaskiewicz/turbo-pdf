...
turbo-pdf ships four JS-facing surfaces:

- **`@turbo-pdf/napi`** — the Node native binding (`compile`/`render` → PDF
  `Buffer`).
- **`@turbo-pdf/wasm`** — the WebAssembly binding (same pipeline in the browser).
- **`@turbo-pdf/react`** — author templates as React components.
- **`@turbo-pdf/template`** — author templates with plain functions (no React).
...
authoring time** (via `renderToStaticMarkup`) into a turbo-pdf template *source
...
```ts
import { compileTemplate } from '@turbo-pdf/react'

const source = compileTemplate(<InvoiceDoc />)   // string: HTML + Jinja + t: directives
// → hand `source` to @turbo-pdf/napi or /wasm `compile(...)`
```
